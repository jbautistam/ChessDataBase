...
namespace Bau.ChessDataBase.Studio.Controllers
{
	...
	/// </summary>
	public class ChessDataBaseStudioController : Libraries.ChessDataBase.ViewModels.Controllers.IChessDataBaseStudioController
	{
		// Eventos públicos
		public event EventHandler<IDetailViewModel> OpenWindowRequired;

		public ChessDataBaseStudioController(AppController appController, string applicationName, MainWindow mainWindow, string appPath)
		{
		...
		public SystemControllerEnums.ResultType OpenDialog(Libraries.BauMvvm.ViewModels.Forms.Dialogs.BaseDialogViewModel dialogViewModel)
		{
			SystemControllerEnums.ResultType result = SystemControllerEnums.ResultType.No;
				...
				{
					case Libraries.ChessDataBase.ViewModels.Configuration.ConfigurationViewModel viewModel:
							result = HostHelperController.ShowDialog(MainWindow, new Views.Tools.ConfigurationView(viewModel));
						break;
				}
				// Devuelve el resultado
				return result;
		...
		public void ShowNotification(SystemControllerEnums.NotificationType type, string title, string message)
		{
			//if (AppController.ConfigurationController.ShowWindowNotifications || type == SystemControllerEnums.NotificationType.Error)
				AppController.ChessDataBaseController.HostController.SystemController.ShowNotification(type, title, message, TimeSpan.FromSeconds(5));
			//else
			//	AppController.ChessDataBaseController.Logger.Default.LogItems.Add(new Libraries.LibLogger.Models.Log.LogModel(null, Libraries.LibLogger.Models.Log.LogModel.LogType.Info,
			//																												  title + ". " + message));
		}
		...
		/// </summary>
		public void LoadConfiguration(ConfigurationViewModel viewModel)
		{
			AppController.ConfigurationController.Load(viewModel);
		}

		/// <summary>
		///		Graba la configuración
		/// </summary>
		public void SaveConfiguration(ConfigurationViewModel viewModel)
		{
			AppController.ConfigurationController.Save(viewModel);
		}
