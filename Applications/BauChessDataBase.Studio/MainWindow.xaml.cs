...
using Bau.ChessDataBase.Studio.Views.Files;

namespace Bau.ChessDataBase.Studio
{
	/// <summary>
	...
	public partial class MainWindow : Window
	{
		...
		{	
			...
			DataContext = ViewModel = new MainViewModel(MainController.ChessDataBaseController);
			ViewModel.Init();
		...
		{
			OpenWindow(new Libraries.ChessDataBase.ViewModels.Solutions.Details.Files.FileViewModel(ViewModel.SolutionViewModel, fileName));
		}
		...
		private void OpenWindow(IDetailViewModel detailsViewModel)
		{
			...
			{
				case Libraries.ChessDataBase.ViewModels.Solutions.Details.Files.ImageViewModel viewModel:
						AddTab(new ImageView(viewModel), viewModel);
					break;
				case Libraries.ChessDataBase.ViewModels.Solutions.Details.Files.FileViewModel viewModel:
		...
		private void CloseWindow(Controls.DockLayout.EventArguments.ClosingEventArgs args)
		{
			...
			{
				Libraries.BauMvvm.ViewModels.Controllers.SystemControllerEnums.ResultType result = MainController.ChessDataBaseController.HostController.SystemController.ShowQuestionCancel
			...
			int indexEnd = mnuWorkspace.Items.IndexOf(mnuEndWorkspaces);
				...
				for (int index = startIndex; index < indexEnd; index++)
					if (mnuWorkspace.Items[index] is MenuItem child && child.Tag is string file)
						child.IsChecked = file.EndsWith(ViewModel.ConfigurationViewModel.Workspace + ".xml", StringComparison.CurrentCultureIgnoreCase);
		}
		...
		private void ExitApp()
		{
			...
			MainController.ConfigurationController.Save(ViewModel.ConfigurationViewModel);
