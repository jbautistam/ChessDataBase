	...
	{	
		...
		{ 
			...
			NewFileCommand = new BaseCommand(_ => CreateFile(), _ => CanCreateFileOrFolder())
										.AddListener(this, nameof(SelectedNode));
			EditFileCommand = new BaseCommand(_ => OpenFile(true), _ => CanExecuteAction(nameof(OpenCommand)))
										.AddListener(this, nameof(SelectedNode));
		...
		///		Abre un archivo
		/// </summary>
		private void OpenFile(bool edit)
		{
			if (SelectedNode is NodeFileViewModel node && !node.IsFolder)
			{
				if (node.FileName.EndsWith(".pgn", StringComparison.CurrentCultureIgnoreCase))
				{
					if (edit)
						...
						SolutionViewModel.MainViewModel.MainController.OpenWindow(new Details.Games.GameBoardPgnViewModel(SolutionViewModel, node.FileName));
				}
		...
		{
			foreach (Interfaces.IDetailViewModel viewModel in SolutionViewModel.MainViewModel.MainController.GetOpenedDetails())
		...
		/// </summary>
		private bool MustClose(Interfaces.IDetailViewModel viewModel, string fileName, bool isFolder)
		{
		...
		/// <summary>
		///		Comando para editar un archivo (lo abre en modo edición, no visualizando la partida)
		/// </summary>
		public BaseCommand EditFileCommand { get; }
