...
namespace Bau.Libraries.ChessDataBase.ViewModels.Tools.Search
{
	/// <summary>
	...
	public class TreeSearchFilesResultViewModel : BaseObservableObject
	{
																							...
																							bool useRegex, CancellationToken cancellationToken)
		{
			...
			string [] lines = await System.IO.File.ReadAllLinesAsync(fileName, cancellationToken);
			int actual = 1;
			Regex regex = GetRegEx(textSearch, caseSensitive, wholeWords, useRegex);

				// Indica el archivo buscado
				FileSearched = System.IO.Path.GetFileName(fileName);
		...
		{
			if (SelectedNode != null && !string.IsNullOrWhiteSpace(SelectedNode.FileName) && System.IO.File.Exists(SelectedNode.FileName))
			{
				// Abre la ventana del archivo
				SearchFilesViewModel.MainViewModel.MainController.OpenWindow(new Solutions.Details.Files.FileViewModel(SearchFilesViewModel.MainViewModel.SolutionViewModel,
																													   SelectedNode.FileName));
				// Va a la línea adecuada (si estamos en el editor del archivo adecuado)
				if (SelectedNode.Line >= 0 &&
						SearchFilesViewModel.MainViewModel.SelectedDetailsViewModel is Solutions.Details.Files.FileViewModel fileViewModel && 
