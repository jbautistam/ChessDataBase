...
namespace Bau.Libraries.ChessDataBase.ViewModels.Tools
{
	...
	public class LastFilesListViewModel : BauMvvm.ViewModels.BaseObservableObject
	{
		...
		public LastFilesListViewModel(Configuration.ConfigurationViewModel configurationViewModel)
		{
				...
				List<string> temporalFiles = SelectFiles();
					...
					foreach (string file in temporalFiles)
						if (temporalFiles.IndexOf(file) < 10 && System.IO.File.Exists(file))
							Files.Add(new LastFileViewModel(ConfigurationViewModel.MainViewModel, file, temporalFiles.IndexOf(file) + 1));
			}
		}
